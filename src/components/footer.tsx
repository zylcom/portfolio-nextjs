...
export default function Footer() {
  ...
  return (
    <motion.footer className="bg-eerie-black" ref={target}>
      <motion.div className="flex min-h-[100vh] w-full flex-col items-start overflow-hidden bg-eerie-black pt-20 text-lighthouse" style={{ y: translateY }}>
        <span className="block h-2.5 w-2.5 self-center rounded-full bg-white" />
        <span className="mt-20 block self-center rounded-full border border-lighthouse px-8 py-3 text-center text-xs uppercase">Have a project in mind?</span>
        ...
        <CursorHover message="Email" scaleSize={6}>
          <motion.div className="relative my-16 grid w-full grid-cols-[repeat(2,minmax(100vw,max-content))] text-center">
            {[0, 1].map((i) => (
              <motion.div className="relative flex animate-marquee-left items-center justify-around" key={i}>
                <Link href="mailto:sabilillah272@gmail.com" className="hidden whitespace-nowrap text-6xl uppercase sm:text-7xl md:block md:text-8xl">
                  Let&#700;s Talk
                </Link>

                <Link href="mailto:sabilillah272@gmail.com" className="block whitespace-nowrap text-6xl uppercase sm:text-7xl md:text-8xl">
                  Let&#700;s Talk
                </Link>
              </motion.div>
            ))}
          ...
          <div className="mx-10 mb-16 flex gap-x-4 md:mb-24">
            ...
            <div>
              <div>
                ...
                <span className="block text-xs text-lighthouse/50">Web Developer</span>
              </div>
